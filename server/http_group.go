...
func (GroupHandler) Insert(r *HTTPHandler) {
	...
	for cmd := range groups[0].IterCommands() {
		ch, err := NewCheckFromDB(r.Server.GetDatabase(), stmt, cl.GetID(), cmd)
		if err != nil {
			r.Server.GetLogger().Error(err, "Internal error")
			r.Output(APIResponse{Error: true, Message: "Internal error"})
			return
		}
		ch.SetGroup(groups[0])
		cl.AddCheck(ch)
	}
...
func (g GroupHandler) Update(r *HTTPHandler) {
	if r.Request.GroupID == -1 {
		r.Output(APIResponse{Error: true, Message: "You need to supply a Group ID"})
		return
	}
